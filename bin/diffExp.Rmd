...
```{r config}
config <- yaml.load_file(here("bin/config.yaml"))
annotation <- read_tsv(here("bin/annotation.tsv"))
```
...
```{r DE_function}
run_de <- function(DE_obj, contrast_name){
  # establish variables
  contrast_tbl = read_tsv(here(config$contrasts))
  contrast_list = contrast_tbl %>% dplyr::filter(name==contrast_name) %>% .[,2:4] %>% unlist(use.names = FALSE)
  # run DE
  DE_obj <- findDEGs(DE_obj,
                      de_pkg = "edger",
                      test = "glmTreat",
                      design = "~0+group",
